...
class CreateUser(TestCase):
	...
	def setUp(self):
		User.objects.create(username="a", email="hehe@virginia.edu", first_name="first", last_name="last");
...
class LoginTest(TestCase):
	...
	def setUp(self):
		...
		self.user1=self.userProfile.objects.create_user(username="user1", email="user1@virginia.edu")
		self.server=UserSocialAuth.objects.create(user=self.user1, provider="google-oauth2", uid="1234")
		self.client=Client()
